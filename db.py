...
async def resolve_equipment(vessel_id: str, name_hint: Optional[str]) -> Optional[Dict[str, Any]]:
...
async def get_crew_profile(engineer_id: str) -> Optional[Dict[str, Any]]:
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        cursor = await db.execute(
...
async def get_checklist_for_vessel_class(vessel_class: str, equipment_id: Optional[str]) -> List[Dict[str, Any]]:
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        if equipment_id:
            cursor = await db.execute(
...
async def insert_equipment_log(
    log_id: str, equipment_id: str, thread_id: str, engineer_id: str,
    raw_entry: str, structured_summary: Dict[str, Any], log_type: str, confidence: float,
) -> None:
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            """INSERT INTO equipment_logs
               ...
               VALUES (?, ?, ?, ?, ?, ?, ?, ?)""",
            (log_id, equipment_id, thread_id, engineer_id, raw_entry,
             json.dumps(structured_summary), log_type, confidence),
        )
        await db.commit()


async def insert_atex_event(
    event_id: str, tenant_id: str, vessel_id: str, equipment_id: str, thread_id: str,
    zone_class: str, trigger_reading: Dict[str, Any], severity: str,
) -> None:
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            """INSERT INTO atex_hazard_events
               ...
               VALUES (?, ?, ?, ?, ?, ?, ?, ?)""",
            (event_id, tenant_id, vessel_id, equipment_id, thread_id, zone_class,
             json.dumps(trigger_reading), severity),
        )
        await db.commit()


async def search_documents(vessel_class: str, equipment_id: Optional[str], query: str) -> List[Dict[str, Any]]:
...
async def update_crew_profile(engineer_id: str, tenant_id: str, vessel_class: Optional[str] = None, equipment_model: Optional[str] = None) -> None:
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        ...
        if row:
            ...
            await db.execute(
                ...
                (f"crew-profile-{engineer_id}", engineer_id, tenant_id, json.dumps(known_classes), json.dumps(known_models))
            )
        await db.commit()
