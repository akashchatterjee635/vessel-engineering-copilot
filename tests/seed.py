...
def seed():
    ...
    conn.execute(
        "INSERT INTO tenants (id, name, fleet_operator) VALUES (?, ?, ?)",
        (TENANT_ID, "Test Fleet Ops", "Test Shipping Co"),
    )

    conn.execute(
        """INSERT INTO vessels (id, tenant_id, name, imo_number, vessel_type, vessel_class, build_date)
                    VALUES (?, ?, ?, ?, ?, ?, ?)""",
        (
            VESSEL_ID,
            TENANT_ID,
    ...
    conn.execute(
        """INSERT INTO threads (id, tenant_id, vessel_id, engineer_id, title)
                    VALUES (?, ?, ?, ?, ?)""",
    ...
    conn.execute(
        ...
        (
            PUMP_EQUIPMENT_ID,
            VESSEL_ID,
            "Aux Boiler Feed Pump #2",
            "propulsion",
            "sub_critical",
            "Sulzer",
            "AHLSTAR-APP",
            "doc://manuals/sulzer-ahlstar-app",
        ),
    )
    ...
    conn.execute(
        ...
        (
            ...
            "Framo",
            "SD125",
            "doc://manuals/framo-sd125",
        ),
    )

    # Crew profile — engineer has worked Suezmax, VLCC; does NOT know Aframax or Framo cargo pumps
    conn.execute(
        """INSERT INTO crew_profiles (id, engineer_id, tenant_id, known_vessel_classes, known_equipment_models)
                    VALUES (?, ?, ?, ?, ?)""",
        (
            "crew-profile-001",
            ENGINEER_ID,
            ...
            json.dumps(["Sulzer/AHLSTAR-APP"]),
        ),
    )  # knows the aux pump model, not the cargo pump

    # Checklist template for this vessel class
    conn.execute(
        """INSERT INTO checklist_templates (id, equipment_id, vessel_class, title, items_json)
                    VALUES (?, NULL, ?, ?, ?)""",
        (
            "checklist-001",
            "Aframax",
            ...
            json.dumps(
                [
                    "Check bilge levels",
                    "Inspect aux pump seals",
                    "Log ambient engine room temp",
                ]
            ),
        ),
    )
