...
import graph as g
import db
from seed import TENANT_ID, VESSEL_ID, ENGINEER_ID, THREAD_ID, PUMP_EQUIPMENT_ID, CARGO_PUMP_EQUIPMENT_ID
...
def make_fake_llm(default_text="Mock synthesis: assessment generated from available context."):
...
async def run_scenario(name, user_query, triage_decision, log_extraction=None, engineer_id=ENGINEER_ID):
    ...
    state = {
        "tenant_id": TENANT_ID, "vessel_id": VESSEL_ID, "vessel_class": "Aframax",
        "thread_id": THREAD_ID, "engineer_id": engineer_id, "user_query": user_query,
        "turn_id": turn_id,
    }
    ...
    try:
        ...
        if "work_order_created" not in result and triage_decision.urgency in ("elevated", "urgent"):
            print("(paused before ActionAgent -- simulating human approval, resuming)")
            result = await g.app.ainvoke(None, config=cfg)
        print("Graph completed. Interaction type:", result["triage_decision"]["interaction_type"])
        ...
        if result.get("onboarding_result"):
            ...
            print("Unfamiliar equipment:", result["onboarding_result"]["unfamiliar_equipment"])
        if result.get("checklist_result"):
            print("Checklist templates found:", len(result["checklist_result"]["templates"]))
...
async def main():
    ...
    await run_scenario(
        ...
        g.TriageDecision(
            interaction_type="diagnostic", urgency="elevated", equipment_hint="aux boiler feed pump",
            needs_telemetry=True, needs_compliance_lookup=False, needs_historical_rag=True,
            needs_weather_voyage=False, needs_maps=False, confidence=0.9,
    ...
    await run_scenario(
        ...
        g.TriageDecision(
            interaction_type="logbook_entry", urgency="routine", equipment_hint="aux boiler feed pump",
            needs_telemetry=False, needs_compliance_lookup=False, needs_historical_rag=False,
            needs_weather_voyage=False, needs_maps=False, confidence=0.95,
    ...
    await run_scenario(
        ...
        g.TriageDecision(
            interaction_type="onboarding_check", urgency="routine", equipment_hint=None,
            needs_telemetry=False, needs_compliance_lookup=False, needs_historical_rag=False,
            needs_weather_voyage=False, needs_maps=False, confidence=0.9,
    ...
    await run_scenario(
        ...
        g.TriageDecision(
            interaction_type="checklist_request", urgency="routine", equipment_hint=None,
            needs_telemetry=False, needs_compliance_lookup=False, needs_historical_rag=False,
            needs_weather_voyage=False, needs_maps=False, confidence=0.9,
    ...
    await run_scenario(
        ...
        g.TriageDecision(
            interaction_type="diagnostic", urgency="elevated", equipment_hint="Cargo Pump #1",
            needs_telemetry=False, needs_compliance_lookup=False, needs_historical_rag=False,
            needs_weather_voyage=False, needs_maps=False, confidence=0.85,
    ...
    await run_scenario(
        ...
        g.TriageDecision(
            interaction_type="port_assistance", urgency="routine", equipment_hint=None,
            needs_telemetry=False, needs_compliance_lookup=False, needs_historical_rag=False,
            needs_weather_voyage=True, needs_maps=True, confidence=0.9,
    ...
    async def hot_telemetry(tenant_id, vessel_id, equipment_id):
        return {"equipment_id": equipment_id, "vibration_amplitude": 9.1,
                "frequency_band": "high", "gas_reading_pct_lel": 25.0}
    ...
    try:
        await run_scenario(
            ...
            g.TriageDecision(
                interaction_type="diagnostic", urgency="urgent", equipment_hint="Cargo Pump #1",
                needs_telemetry=False, needs_compliance_lookup=False, needs_historical_rag=False,
                needs_weather_voyage=False, needs_maps=False, confidence=0.8,
    ...
    for table in ["equipment_logs", "atex_hazard_events", "work_orders", "tool_call_audit"]:
